...
use App\Http\Repositories\Frontend\CheckoutApi;
...
{
    public function __construct(CartController $cartController, Cart $cart, UsersApis $usersApis, CheckoutApi $checkoutApi, PromoController $promoController, CartRepository $cartRepository)
    {
    ...
    public function saveShippingDelivery(Request $request)
    {
        ...
        if(Auth::check())
        {
            ...
            { 
              ...
              $this->checkoutApi->updateAddress($request); 
  ...
  {  
      ...
      else
      {
        ...
        {
          // check if the cart is not empty and the total is different from 0
          if(!empty($cart_data['items']) && $totals['total_float'] != 0)
          {
            //====== A ========= INSERT ORDER + send email + Empty card
            $order_id = $this->insertOrder($request, $totals, $cart_data);
  ...
  // ====== A ========= Insert order + send email
  public function insertOrder($request, $totals, $cart_data)
  {
    // if the session variables exist    
    if(session('email') !== NULL && session('address') !== NULL  && session('country') !== NULL)  
    {     
      // insert the order in the DB 
      $order_id = $this->checkoutApi->insertOrder($request, $cart_data['items'], $totals['shipping_fees_float'], $totals['promo_type'], $totals['promo_value'], $totals['subtotal_float'], $totals['total_float']);

      // ====== B ========= send email on purchase
      $this->sendPurchaseEmail($cart_data, $totals, $order_id); 
  ...
  // ====== B ========= send email on purchase
  public function sendPurchaseEmail($cart_data, $totals, $order_id)
  {
    $purchase['date'] = Carbon::now(config('global.TIMEZONE'))->format("d M Y");
    $purchase['time'] = Carbon::now(config('global.TIMEZONE'))->format("h:i a");
    $shipping_country = $this->checkoutApi->getCountryFromId(session('country')); // get the name of the country - shipping address
    
    if(session('billing_country'))
      $billing_country = $this->checkoutApi->getCountryFromId(session('billing_country')); // get the name of the country - billing address
    else
      $billing_country = NULL;

    // send email to client
    Mail::send('emails.cash-on-delivery-client-email', array('cart_data' => $cart_data, 'totals' => $totals, 'order_id' => $order_id, 'purchase' => $purchase, 'shipping_country' => $shipping_country, 'billing_country' => $billing_country), function($message)
    {
    ...
    // send email to admin
    Mail::send('emails.cash-on-delivery-admin-email', array('cart_data' => $cart_data, 'totals' => $totals, 'order_id' => $order_id, 'purchase' => $purchase, 'shipping_country' => $shipping_country, 'billing_country' => $billing_country, 'client_email' => session('email')), function($message)
    {
