...
class CartControllerAramex extends Controller
{
    public function __construct(ShoppingCartAPis $shoppingCartApis, ProductApis $productApis, Cart $cart, UsersApis $usersApis, PromoController $promoController, AramexController $aramexController)
    {
    ...
    {
      // if aramex delivery API is enabled
      if(config('global.ARAMEX_SHIPPING_APIS') == 1)
      {
        $gross_weight = 0;
        $total_quantity = 0;

        // loop the cart and calculate the gross weight
        foreach($cart_items as $c)
        {
          $gross_weight = $gross_weight + ($c->weight * $c->quantity);
          $total_quantity = $total_quantity + $c->quantity;
        }
        // get the shipping price from aramex API
        $aramex_shipping_price = $this->aramexController->getAramexRate($gross_weight, $total_quantity);

        return $aramex_shipping_price;
      }

      // ARAMEX API is disabled
      else
      {
