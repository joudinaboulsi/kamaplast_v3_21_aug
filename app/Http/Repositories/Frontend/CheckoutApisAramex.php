...
class CheckoutApisAramex {
    ...
    Insert an order
    ------------------------------------*/
    public function insertOrder($request, $cart_data, $shipping_fees, $promo_type, $promo_value, $subtotal, $total, $aramex_response)
    {
        \DB::transaction(function () use ($request, $cart_data, $shipping_fees, $promo_type, $promo_value, $subtotal, $total, $aramex_response)
        {  
          ...
          else // no promo_applied
          {
           ...
           $order_id =  \DB::table('orders')->insertGetId(
               array('order_status_id' => 1,
                     ...
                     'aramex_tracking_number' => $aramex_response['data']->Shipments->ProcessedShipment->ID,
                     'aramex_foreign_ref' => $aramex_response['data']->Shipments->ProcessedShipment->ForeignHAWB,
                     'aramex_pdf_receipt' => $aramex_response['data']->Shipments->ProcessedShipment->ShipmentLabel->LabelURL,
    ...
    // get last aramex Foreign ref 
    public function getLastAramexForeignRef()
    {
        // initial foreign ref in case it is the first shipment orders
        $initial_value = 999;

        $q = \DB::select("SELECT aramex_foreign_ref
                         FROM orders
                         ORDER BY aramex_foreign_ref DESC
                         LIMIT 0,1");

        if(empty($q))
          return $initial_value; 
        
        else if($q[0]->aramex_foreign_ref == NULL)
          return $initial_value;
        
        else
         return $q[0]->aramex_foreign_ref;
    }
